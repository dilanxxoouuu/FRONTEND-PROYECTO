...
const Login = () => {
  ...
  const [showToast, setShowToast] = useState(false); // Nueva variable para el toast
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Esperamos 2 segundos para que el toast se vea antes de redirigir
          setTimeout(() => {
            setShowToast(false);
            switch (data.rol) {
              case 1:
  ...
  return (
    <div className="login-container">
      ...
      {showToast && (
        <div className="login-toast success-toast">
          <span className="toast-icon">✅</span> Inicio de sesión exitoso
        </div>
      )}
    </div>
  );
